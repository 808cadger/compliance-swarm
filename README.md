...
shared/
- model-client.js — settings (localStorage, versioned), policy loading, config resolution, and dispatch, imported by all 6 agent pages as an ES module
- providers/anthropic.js, providers/ollama.js — pure request/response adapters for each provider
- agent-common.css, agent-common.js — CSS shared across the agent pages, plus HTML-escaping and the postMessage helpers (notifyParentIfEmbedded/listenForDecisions) used by Payroll/Books/Contract to talk to the orchestrator
