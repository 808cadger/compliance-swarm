...
- **Ruling (human, pre-flight, 2026-08-07):** the design spec's text says `callModel` "re-checks `allowedProviders`" as defense in depth, but `callModel`'s signature is fixed at `(config, messages)` and never receives `modelPolicy`. Task 3's actual implementation is the ruled-correct one: `callModel` only verifies `config.provider` is a recognized dispatch target (`'anthropic'` or `'ollama'`, else `UNKNOWN_PROVIDER`) — `resolveModelConfig` remains the sole enforcer of `allowedProviders`. This is not a spec deviation to flag or fix; do not add a `modelPolicy` parameter to `callModel`.
